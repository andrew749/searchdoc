...
type Options struct {
	...
	MaxLineLength int // Sets the point at which to break wrappable lines in the output.
}
...
func (ctx *textifyTraverseContext) breakLongLines(data string) []string {
	...
	//}
	lineLen := maxLineLen
	if ctx.options.MaxLineLength > 0 {
		lineLen = ctx.options.MaxLineLength
	}
	var (
		ret      = []string{}
		runes    = []rune(data)
